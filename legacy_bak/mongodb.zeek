...
export {
	...
	option max_buffered_bytes = 1024 * 1024;
	option max_message_size = 16 * 1024 * 1024;
}
...
type State: record {
	orig_buf: string &default="";
	resp_buf: string &default="";
...
redef tcp_content_delivery_ports_orig += { [27017/tcp] = T };
redef tcp_content_delivery_ports_resp += { [27017/tcp] = T };
...
function log_request(c: connection, request_id: count, opcode: string, parsed: table[string] of string)
	{
	if ( max_pending_requests > 0 && |c$mongodb_state$pending| >= max_pending_requests )
		{
		reset_state(c, "MongoDB_excessive_pipelining", fmt("%d", |c$mongodb_state$pending|));
		return;
		}
...
function process_buffer(c: connection, is_orig: bool)
	{
	local buf = is_orig ? c$mongodb_state$orig_buf : c$mongodb_state$resp_buf;

	while ( |buf| >= 16 )
		{
		local msg_len = le_u32(buf, 0);
		if ( msg_len < 16 )
			{
			reset_state(c, "MongoDB_invalid_message_length", fmt("%d", msg_len));
			return;
			}

		if ( max_message_size > 0 && msg_len > max_message_size )
			{
			reset_state(c, "MongoDB_excessive_message_size", fmt("%d", msg_len));
			return;
			}
		...
		local request_id = le_u32(buf, 4);
		local response_to = le_u32(buf, 8);
		local opcode_num = le_u32(buf, 12);
		local opcode = op_name(opcode_num);
		local frame = slice(buf, 0, msg_len);
		local body = slice(frame, 16, msg_len - 16);

		if ( opcode_num == 2013 )
			{
			if ( ! parse_op_msg_body(c, is_orig, request_id, response_to, opcode, body) )
				{
				reset_state(c, "MongoDB_malformed_op_msg", fmt("%s %d", is_orig ? "orig" : "resp", msg_len));
				return;
				}
			}
		else
			log_opaque_message(c, request_id, response_to, opcode, is_orig);

		buf = slice(buf, msg_len, |buf| - msg_len);
		}

	if ( is_orig )
		c$mongodb_state$orig_buf = buf;
	else
		c$mongodb_state$resp_buf = buf;
	}
...
event tcp_contents(c: connection, is_orig: bool, seq: count, contents: string)
	...
	if ( is_orig )
		c$mongodb_state$orig_buf = cat(c$mongodb_state$orig_buf, contents);
	else
		c$mongodb_state$resp_buf = cat(c$mongodb_state$resp_buf, contents);

	if ( max_buffered_bytes > 0 )
		{
		local current_len = is_orig ? |c$mongodb_state$orig_buf| : |c$mongodb_state$resp_buf|;
		if ( current_len > max_buffered_bytes )
			{
			reset_state(c, "MongoDB_excessive_buffering", fmt("%d", current_len));
			return;
			}
		}

	process_buffer(c, is_orig);
	}
...
event content_gap(c: connection, is_orig: bool, seq: count, length: count)
	...
	reset_state(c, "MongoDB_content_gap", fmt("%s %d", is_orig ? "orig" : "resp", length));
	}
